...
std::string Renderer::render(const Scene& scene, const Camera& cam) const {
  ...
  for (int y = 0; y < height; ++y) {
    for (int x = 0; x < width; ++x) {
      ...
      // use a smaller charAspect to compress vertical axis and reduce tall ovals
      ...
      if (scene.traceSDF(ro, rd, t, normal, mat)) {
        ...
        if (mat == 2) matFactor = 0.8f; // cylinder
        if (mat == 3) matFactor = 1.2f; // torus
        if (mat == 4) matFactor = 0.6f; // plane
        if (mat == 5) matFactor = 1.0f; // tetra
        float shadeVal = std::clamp(lum * matFactor, 0.0f, 1.0f);
        int idx = (int)(shadeVal * (asciiChars.size() - 1));
        idx = std::clamp(idx, 0, (int)asciiChars.size() - 1);
        ...
        if (mat == 4) {
          ...
          if (isDot && fade > 0.12f) out.push_back('.'); else out.push_back(' ');
        } else {
